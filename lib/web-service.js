...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function(args, callback) {
    ...
    if (args.strUserName.trim() === username && args.strPassword.trim() === password) { 
        //TODO FOR RYAN: replace this condition with a query to the database for a matching username and password. 
        //if there is one found, proceed into this if block, and update "username" and "password" from their default values to the ones in the database
        //then, install https://www.npmjs.com/package/node-localstorage localstorage on nodejs, and then put the username and password into localstorage
        //then, replace all occurrences of process.env.QB_USERNAME and process.env.QB_PASSWORD in other files (i think its just client.js) with references to the localstorage values
        //then, get from the database that user's company path file and update "companyFile" as well
        //if there is no corresponding username/password pair, skip over to the else block below
        //otherwise, pull from the database the corresponding xmls (should be stored in the database as a bunch of strings), and populate the qbXMLHandler folder with new files (1.xml, 2.xml, etc. in order of the xmls, filling each file with each corresponding xml string from the database), removing any old xml files as well that might conflict

        //TODO FOR RYAN: create a database. each user needs to have a username/password, a companyFilePath, and corresponding XMLs (just really long strings lol)
