...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function(args, callback) {
    ...
    if (/* TODO FOR RYAN: IF DATABASE CONTAINS USERNAME args.strUserName.trim() AND PASSWORD args.strPassword.trim()*/true) { 
        username = args.strUserName.trim();
        password  = args.strPassword.trim();
        
        reactLocalStorage.set('username', args.strUserName.trim());
        reactLocalStorage.set('password', args.strPassword.trim());
        
        //TODO FOR RYAN: then, get from the database that user's company path file and update the "companyFile" variable to be that
        
        //TODO FOR RYAN: pull from the database the corresponding xmls (should be stored in the database as a bunch of strings), and put them in localstorage as well
