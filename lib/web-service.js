...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function(args, callback) {
    ...
    if (args.strUserName.trim() === username && args.strPassword.trim() === password) { 
        ...
        //if there is no corresponding username/password pair, skip them over to the else block below
        //then, pull from the database the corresponding xml files, and populate the qbXMLHandler folder with them, removing any old ones

        //TODO FOR RYAN: create a database. each user needs to have a username/password, a companyFilePath, and corresponding XML files
